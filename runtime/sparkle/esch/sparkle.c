...
void esch_sparkle(uint32_t *state, int brans, int steps)
{
...
void esch_sparkle_inv(uint32_t *state, int brans, int steps)
{
  int i, j;  // Step and branch counter
...
void esch_clear_state(uint32_t *state, int brans)
{
  int i;
...
void esch_print_state(const uint32_t *state, int brans)
{
...
void esch_test_sparkle(int brans, int steps)
{
  uint32_t state[2*MAX_BRANCHES] = { 0 };
  
  printf("input:\n");
  esch_print_state(state, brans);
  esch_sparkle(state, brans, steps);
  printf("sparkle:\n");
  esch_print_state(state, brans);
  esch_sparkle_inv(state, brans, steps);
  printf("sparkle inv:\n");
  esch_print_state(state, brans);
  printf("\n");
}
